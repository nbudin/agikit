...
function processFile<T>(processor: (input: string) => T, filePath: string) {
  ...
  try {
    ...
    if (
      error instanceof LogicSyntaxError ||
      error instanceof WordListSyntaxError
    ) {
...
export function buildGame(root: string): void {
  ...
  if (fs.existsSync(explicitVolumesPath)) {
    ...
    const explicitVolumeData = JSON.parse(
      fs.readFileSync(explicitVolumesPath, "utf-8")
    );
    explicitVolumes = explicitVolumeData.map(
      (volumeData: {
        ...
        for (const resourceType of [
          ...
          volumeData.resources[resourceType]?.forEach(
            (resourceNumber: number) => {
  ...
  fs.writeFileSync(
    path.join(destinationDir, "WORDS.TOK"),
    buildWordsTok(wordList)
  );

  console.log(`Writing OBJECT`);
  fs.writeFileSync(
    path.join(destinationDir, "OBJECT"),
    buildObjectList(objectList)
  );
}
