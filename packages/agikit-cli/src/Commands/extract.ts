...
import {
  readWordsTok,
  exportWords,
} from "agikit-core/dist/Extract/ReadWordsTok";
  ...
  if (entry.resourceType === ResourceType.LOGIC) {
    ...
    writeFileSync(
      path.join(
        destDir,
        entry.resourceType.toLowerCase(),
        `${entry.resourceNumber}.agiasm`
      ),
      generateLogicAsm(logic, wordList)
    );

    const [code, basicBlockGraph] = generateCodeForLogicResource(
      logic,
      wordList
    );
